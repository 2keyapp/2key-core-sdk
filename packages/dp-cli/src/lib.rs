...
//!
//! Product tenants (IDR, …) depend on this crate and bake `DP_PRODUCT_NAME` /
//! `DP_BACKEND_URL` at compile time. Bins in this package stay for local tests.
...
struct Cli {
    /// Billing Machine AuthN base URL (`…/api/v1`). Overrides the compiled default.
    #[arg(long, global = true, env = "DP_BACKEND_URL")]
    backend_url: Option<String>,

    /// Better Auth mount (`…/api/auth`). Default: derived from `--backend-url`.
    #[arg(long, global = true, env = "DP_AUTH_URL")]
...
/// Lifecycle CLI (`idr` / `dp-cli`): signup, register, CSR inbox. Exits when done.
pub fn lifecycle_main() -> ExitCode {
    let cli = parse_cli();
    let cfg = ResolvedConfig::from_env();
    match run_lifecycle(cli, cfg) {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
...
/// Resident agent (`idr-agent` / `dp-agent`): load endorsed cert and stay up.
...
#[tokio::main]
async fn run_lifecycle(cli: Cli, mut cfg: ResolvedConfig) -> dp_rust_sdk::Result<()> {
    if let Some(url) = &cli.backend_url {
        cfg.set_backend_url(url.clone());
    }
    if let Some(url) = &cli.auth_url {
        cfg.auth_url = url.clone();
    }
...
pub(crate) fn auth_client(cfg: &ResolvedConfig) -> DpClient {
    let mut client = DpClient::new(&cfg.auth_url).with_user_agent(format!(
        "{}/{}",
        cfg.product_name,
        env!("CARGO_PKG_VERSION")
    ));
